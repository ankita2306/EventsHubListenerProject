...
public class AccountStatusChangeEvent
{
    ...
    @Autowired
    public AccountStatusChangeEvent(Cluster cluster,
                         Bucket bucket,
                         Scope scope,
...
//                                eventsService.consumeAccountStatusChangeBusinessEvents();
                                /*for (Object o : i.equals("phonenumber_change_events") ? eventsService.consumePhoneBusinessEvents(partitionEvent) :
                                        i.equals("new_account_created") ? eventsService.consumeNewAccountCreatedBusinessEvents(partitionEvent) :
                                                i.equals("closed_accounts") ? eventsService.consumeClosedAccountsBusinessEvents(partitionEvent) :
                                                        i.equals("cifid_change_on_accounts") ? eventsService.consumeCifIdChangeOnAccountsBusinessEvents(partitionEvent) :
                                                                i.equals("ucic_change") ? eventsService.consumeUcicReferenceBusinessEvents(partitionEvent) :
                                                                      eventsService.consumeNewCustomerOnboardedBusinessEvents(partitionEvent));*/
...
//                                    else if(i.equals("new_customer_onboarded")) eventsService.consumeNewCustomerOnboardedBusinessEvents();
                               ...
                               switch (i) {
                                    case "phonenumber_change_events":
                                        eventsService.consumePhoneBusinessEvents(partitionEvent);
                                        break;
                                    case "new_account_created":
                                       eventsService.consumeNewAccountCreatedBusinessEvents(partitionEvent);
                                        break;
                                    case "closed_accounts":
                                         eventsService.consumeClosedAccountsBusinessEvents(partitionEvent);
                                        break;
                                    case "cifid_change_on_accounts":
                                      eventsService.consumeCifIdChangeOnAccountsBusinessEvents(partitionEvent);
                                        break;
                                    case "ucic_change":
                                         eventsService.consumeUcicReferenceBusinessEvents(partitionEvent);
                                        break;
                                    default:
                                         eventsService.consumeNewCustomerOnboardedBusinessEvents(partitionEvent);
                                        break;
